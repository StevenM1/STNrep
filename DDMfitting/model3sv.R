...
# Prior distributions
p.mean <- c(a=2, v.low=3, v.high=3, v_bias.biastrial=0, z=0.5, z_bias.biastrial=0.0, t0=0.3, sv=0.1, sz=0.1)
p.scale <-c(a=2, v.low=3, v.high=3, v_bias.biastrial=2, z=0.1, z_bias.biastrial=0.1, t0=0.3, sv=0.5, sz=0.1)
p.prior <- prior.p.dmc(
  dists = rep("tnorm",9),
  p1=p.mean,
  p2=p.scale,
  lower=c(0,-5, -5, -5, .3, -.3, 0, 0, 0),
  upper=c(5, 7,  7,  7, .7,  .3, 1, 5, 0.3)
)
