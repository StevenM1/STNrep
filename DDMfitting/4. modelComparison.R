...
for(modelN in c(1,2,3)) {
  ...
  load(paste0('samplesModel', modelN, '.RData'))
...
save(allBPICs, file='BPICs.RData')
save(allSummaries, file='parameterSummaries.RData')
...
# Ignore model 1 for BMA weighting
...
apply(bmaParameters, 2, mean)

bmaParameters.new <- bmaParameters
write.csv(bmaParameters.new, '../derivatives/BMAParameters.csv')


## effect sizes per dataset?
rtByCoh <- aggregate(RT~coherence*s, dat, mean)
rtByCoh <- reshape(rtByCoh, v.names='RT', timevar='coherence', idvar='s', direction='wide')
rtByCoh$RTdiff <- rtByCoh$RT.low-rtByCoh$RT.high
rtByCoh$dataset <- c(rep(1, 18), rep(2, 15))
aggregate(RTdiff~dataset, rtByCoh, mean)

dat$acc <- as.numeric(dat$R) == as.numeric(dat$S)
accByCoh <- aggregate(acc~coherence*s, dat, mean)
accByCoh <- reshape(accByCoh, v.names='acc', timevar='coherence', idvar='s', direction='wide')
accByCoh$accdiff <- accByCoh$acc.low-accByCoh$acc.high
accByCoh$dataset <- c(rep(1, 18), rep(2, 15))
aggregate(accdiff~dataset, accByCoh, mean)

#tapply(da$RT, dat$coherence, dat$sub, mean)
